...
\date{2026-06-10}
\title{Taller — Maquina IAS / Von Neumann\\\medskip
\large ICCD332 Arquitectura de Computadores}
\hypersetup{
 pdfauthor={Danilo Unapucha},
 pdftitle={Taller — Maquina IAS / Von Neumann},
 pdfkeywords={},
 pdfsubject={},
...
siguientes preguntas considerando la Tabla \ref{tab:org59f9872} como el
mapa de memoria de la máquina y obtenga el programa en lenguaje ensamblador y en
RTL.

\begin{table}[htbp]
\caption{\label{tab:org59f9872}Mapa de Memoria}
\centering
\begin{tabular}{rrrrr}
\hline
...
\label{sec:org5305946}
Procedmiento:
Para la direccion \(\text{0x300}\)

\begin{itemize}
\item \([PC] \leftarrow \text{0x300}\)
\item \([IR] \leftarrow [[PC]]\)
\item \([IR] \leftarrow \text{[0x300]}\)
\item \([IR] \leftarrow \text{0x01940}\)
\item \([AC] \leftarrow \text{[0x940]}\)
\item \([AC] \leftarrow \text{005}\)

\item \([PC] \leftarrow \text{0x300}\)
\item \([IR] \leftarrow [[PC]]\)
\item \([IR] \leftarrow \text{[0x300]}\)
\item \([IR] \leftarrow \text{0x06941}\)
\item \([AC] \leftarrow [AC]-\text{[0x941]}\)
\item \([AC] \leftarrow \text{0x005 - 0x002}\)
\item \([AC] \leftarrow \text{003}\)
\end{itemize}

Para la dirección \(\text{0x301}\)

\begin{itemize}
\item \([PC] \leftarrow \text{0x301}\)
\item \([IR] \leftarrow [[PC]]\)
\item \([IR] \leftarrow \text{[0x301]}\)
\item \([IR] \leftarrow \text{0x21940}\)
\item \(\text{[0x940]} \leftarrow [AC]\)
\item \(\text{[0x940]} \leftarrow \text{003}\)

\item \([PC] \leftarrow \text{0x301}\)
\item \([IR] \leftarrow [[PC]]\)
\item \([IR] \leftarrow \text{[0x301]}\)
\item \([IR] \leftarrow \text{0x14000}\)
\item \([AC] \leftarrow [AC] * 2\)
\item \([AC] \leftarrow \text{006}\)
\end{itemize}

Para la dirección \(\text{0x302}\)

\begin{itemize}
\item \([PC] \leftarrow \text{0x302}\)
\item \([IR] \leftarrow [[PC]]\)
\item \([IR] \leftarrow \text{[0x302]}\)
\item \([IR] \leftarrow \text{0x21941}\)
\item \(\text{[0x941]} \leftarrow [AC]\)
\item \(\text{[0x941]} \leftarrow \text{006}\)

\item \([PC] \leftarrow \text{0x302}\)
\item \([IR] \leftarrow [[PC]]\)
\item \([IR] \leftarrow \text{[0x302]}\)
\item \([IR] \leftarrow \text{0x02940}\)
\item \([AC] \leftarrow \text{-[0x940]}\)
\item \([AC] \leftarrow \text{-003}\)
\end{itemize}
...
\item \([IR] \leftarrow [[PC]]\)
\item \([IR] \leftarrow \text{[0x303]}\)
\item \([IR] \leftarrow \text{0x99000}\)
\item \([IR] \leftarrow \text{HALT}\)
\end{itemize}
...
\item La operación que modifica el bit es SUB M(941)
...
\hline
Danilo Unapucha & danilo.unapucha@epn.edu.ec & Líder\\[0pt]
